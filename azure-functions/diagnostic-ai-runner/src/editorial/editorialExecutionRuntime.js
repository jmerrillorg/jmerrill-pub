...
function outputDefinitions(stageCode) {
  ...
  if (stageCode === "DEVELOPMENTAL_EDITING") {
    return ["Developmental Editing Working Manuscript Ledger", "Developmental Memo", "Change Ledger", "Developmental QA Evidence"];
  }
...
async function materializeEditorialOutputs(client, stage, stageCode, sourceArtifact, correlationId) {
  ...
  for (const outputName of outputDefinitions(stageCode)) {
    ...
    const content = buildOutputDocument(stage, stageCode, sourceArtifact, outputName, extracted.value || "", correlationId);
    ...
    const artifactPayload = {
      ...
      jm1pub_notes: `Editorial runtime output produced from governed source artifact ${sourceArtifact.jm1pub_editorialartifactid}.`,
...
async function processStage(client, stage, correlationId) {
  ...
  const qaLogId = await writeLog(client, {
    ...
    description:
      `${stageCode} QA completed: source checksum matched, source file was materialized, output artifacts were uploaded and linked to the active title/stage. Package assembly remains gated until stage completion and cadence policy allow release. Correlation ${correlationId}.`,
    sourceEntity: "jm1pub_editorialstage",
    sourceRecordId: stage.jm1pub_editorialstageid
  });
  return {

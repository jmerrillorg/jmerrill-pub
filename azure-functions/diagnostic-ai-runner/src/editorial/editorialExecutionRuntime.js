...
const EXECUTOR_POLICIES = {
  ...
  LINE_EDITING: {
    ...
    outputRoles: ["editedManuscript", "reviewCoverNote", "lineEditingSummary", "changeLedger", "qaEvidence"],
...
function outputDefinitions(stageCode) {
  ...
  const displayNames = {
    ...
    reviewCoverNote: "Line Editing Review Notes",
...
async function buildDevelopmentalRevisionDocx(stage, stageCode, sourceArtifact, outputName, extractedText, correlationId) {
  ...
  const children = [
    ...
    paragraphFromText("Developmental Editing Review Copy", { heading: HeadingLevel.HEADING_2 }),
    paragraphFromText(
      "This review copy preserves the manuscript text and includes non-destructive editorial notes where structure, pacing, permissions, or publisher judgment may be needed."
    ),
...
function buildLineEditingReviewNotesText(stage, lineOutput) {
  const notes = [
    `Line Editing Review Notes - ${stage.jm1pub_name}`,
    "",
    "What was completed",
    lineOutput.lineEditingSummary || "Line editing was completed for sentence-level clarity, paragraph flow, rhythm, readability, tone, and author-voice preservation.",
    "",
    "What to review",
    "Please review the attached line-edited manuscript and note anything you want corrected, clarified, or discussed before the project moves forward.",
    "",
    "Change notes",
    ...(lineOutput.changeLedger.length ? lineOutput.changeLedger.map((item) => `- ${item}`) : ["- No recurring line-editing pattern was separately reported."]),
    "",
    "Author questions",
    ...(lineOutput.authorQueries.length ? lineOutput.authorQueries.map((item) => `- ${item}`) : ["- No specific author questions were recorded for this pass."]),
    "",
    "How to respond",
    "Reply with Approved, Approved with corrections, or I have questions. You may include one consolidated correction list in your reply."
  ];
  return notes.join("\n");
}
...
function buildLineEditingMarkdownOutput(stage, outputName, sourceArtifact, extractedText, correlationId, modelInvocation) {
  const { lineOutput, qa } = assertLineEditingOutputReady(modelInvocation, extractedText, correlationId);
  if (outputName === "Line Editing Review Notes") return buildLineEditingReviewNotesText(stage, lineOutput);
  const base = [

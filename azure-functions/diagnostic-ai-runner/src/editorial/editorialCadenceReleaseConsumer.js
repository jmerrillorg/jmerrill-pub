...
function normalizeStageCode(stage) {
  const haystack = [stage?.jm1pub_name, stage?.jm1pub_stagetype, stage?.jm1pub_internaloperationalsummary]
    .map(normalizeString)
    .join(" ")
    .toLowerCase();
  if (haystack.includes("line")) return "LINE_EDITING";
  if (haystack.includes("copy")) return "COPYEDITING";
  if (haystack.includes("proof")) return "PROOFREADING";
  if (haystack.includes("interior") || haystack.includes("layout")) return "INTERIOR_LAYOUT";
  if (haystack.includes("cover")) return "COVER_DESIGN";
  if (haystack.includes("developmental")) return "DEVELOPMENTAL_EDITING";
...
async function getTitle(client, titleId) {
  const rows = await client.list("jm1pub_titles", {
    $select: "jm1pub_titleid,jm1pub_name,jm1pub_titlename,jm1pub_authorname,modifiedon,createdon",
...
async function processCadenceLog(client, cadenceLog, now, correlationId, deps = {}) {
  ...
  if (sent) {
    return {
      ...
      packageId: packageInfo.packageId,
      sentActionType: sent.jm1_actiontype
    };
  }

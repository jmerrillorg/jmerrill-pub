...
function parseStructuredJsonObject(content) {
  ...
  const attempts = [{ candidate: trimmed, type: "direct" }];
  if (trimmed.startsWith("```")) {
    const repaired = trimmed
      .replace(/^```[a-zA-Z0-9_-]*\s*/u, "")
      .replace(/\s*```$/u, "")
      .trim();
    attempts.push({ candidate: repaired, type: "fenced" });
  }

  const extractedJson = extractFirstJsonObject(trimmed);
  if (extractedJson && !attempts.some((attempt) => attempt.candidate === extractedJson)) {
    attempts.push({ candidate: extractedJson, type: "embedded" });
  }
    ...
    try {
      ...
      if (!parsed || Array.isArray(parsed) || typeof parsed !== "object") {
        return {
          ...
          repaired: index > 0,
          classification: classifyNonObjectAttempt(attempt.type)
        };
      }
      return {
        ok: true,
        value: parsed,
        repaired: index > 0,
        classification: classifySuccessfulAttempt(attempt.type)
      };
    } catch {
  ...
  return {
    ...
    repaired: false,
    classification: attempts.some((attempt) => attempt.type === "embedded")
      ? "embedded-invalid"
      : (attempts.some((attempt) => attempt.type === "fenced") ? "fenced-invalid" : "invalid-json")
  };
}

function classifySuccessfulAttempt(type) {
  if (type === "direct") {
    return "direct-json";
  }

  if (type === "embedded") {
    return "embedded-repaired";
  }

  return "fenced-repaired";
}

function classifyNonObjectAttempt(type) {
  if (type === "direct") {
    return "non-object";
  }

  if (type === "embedded") {
    return "embedded-non-object";
  }

  return "fenced-non-object";
}

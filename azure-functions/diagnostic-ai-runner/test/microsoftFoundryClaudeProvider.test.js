...
describe("microsoftFoundryClaudeProvider", () => {
  ...
  test("recovers a valid JSON object when the model wraps it in extra prose", async () => {
    const { loaded, restore } = loadProviderWithStubs({
      fetchImpl: async () => ({
        ok: true,
        status: 200,
        ...
        async json() {
          return {
            choices: [{ message: { content: "Structured result follows.\n{\"ok\":true,\"provider\":\"foundry\"}\nEnd." } }],
    ...
    try {
      await withEnv(
        ...
        async () => {
          ...
          assert.equal(result.ok, true);
          assert.equal(result.output.ok, true);
          assert.equal(result.output.provider, "foundry");
          assert.equal(result.responseClassification, "embedded-repaired");
        }
      );
    } finally {
      restore();

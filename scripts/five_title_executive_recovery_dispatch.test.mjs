...
const dispatchService = readFileSync(new URL('../lib/server/publishing-dispatch-service.ts', import.meta.url), 'utf8')
...
test('executive recovery dispatch cannot send attachmentless or unbranded author-review packages', () => {
  ...
  assert.match(dispatchService, /DUPLICATE_ACTIVE_GATE_RECONCILIATION_REQUIRED/)
  assert.match(dispatchService, /PUBLISHING_DISPATCH_BLOCKED - INTAKE_REFERENCE_CODE_INVALID/)
  assert.match(dispatch, /interiorProof', 'reviewInstructions/)
  assert.match(dispatch, /editedManuscript', 'reviewInstructions/)
  assert.doesNotMatch(dispatch, /return \['editedManuscript', 'editorialMemo', 'reviewInstructions'\]/)
  assert.doesNotMatch(dispatch, /return \['interiorProof', 'reviewInstructions', 'authorResponseMechanism'/)
  assert.doesNotMatch(dispatch, /return \['editedManuscript', 'editorialMemo', 'reviewInstructions', 'authorResponseMechanism'/)
  assert.match(dispatch, /expectedTitleId:\s*'2d21ab5b-4d80-f111-ab0f-7c1e525b15c2'/)
  assert.match(dispatch, /require-operational-delivery-certification-before-author-response-clock/)
  assert.match(dispatch, /!\/instruction\|guide\|review\/i\.test\(haystack\)/)
  assert.match(dispatch, /!\/\\\.pdf\\b\|pdf\/i\.test\(haystack\)/)
  assert.match(dispatch, /role === 'reviewInstructions' && \/editorial\.\*review\.\*guide\|review\.\*guide\/i\.test\(haystack\)/)

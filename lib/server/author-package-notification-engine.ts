...
const INTERNAL_ATTACHMENT_CONTENT_PATTERNS: Array<[string, RegExp]> = [
  ...
  ['AI_MODEL_REFERENCE', /\bAI (?:model|assistant|agent)\b/i],
  ['AUTOMATION_REFERENCE', /\b(?:workflow|publishing) (?:automation|orchestrator)\b/i],
  ['INTERNAL_PLATFORM_REFERENCE', /\b(?:Dataverse|Azure(?:\s+(?:Functions?|Communication Services|ACS))?)\b/i],
  ...
  ['INTERNAL_RIGHTS_NOTE', /\b(?:legal posture|RIGHTS_LEGAL_INTERNAL)\b/i],
  ['INTERNAL_NOTE_CLASS', /\b(?:PUBLISHER|FACT_CHECK|PRODUCTION|PROVIDER|SYSTEM|AI)_INTERNAL\b/i],
  ['BEFORE_AUTHOR_FACING_RELEASE_NOTE', /\bbefore author-facing release\b/i],
  ['EXECUTION_ID_EXPOSED', /\bexecution\s*id\b/i],
  ['CAPABILITY_ID_EXPOSED', /\bcapability\s*id\b/i],
  ['INTERNAL_STAGE_LABEL', /\b(?:DEVELOPMENTAL_EDITING|LINE_EDITING|COPYEDITING|LAYOUT_TYPESETTING|PROOFREADING|AUTHOR_PROOF|FINAL_PRODUCTION)\b/i],

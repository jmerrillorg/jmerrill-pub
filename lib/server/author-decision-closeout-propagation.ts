...
export async function propagateAuthorDecisionEvidence(input: PropagationInput): Promise<PropagationResult> {
  ...
  const approvalGate = evaluateAuthorFinalApprovalGate({
    ...
    currentStageArtifactId: selectedArtifact?.artifactId,
    approvedArtifactId: approvedDecision ? selectedArtifact?.artifactId : undefined,
    ...
    currentStageArtifactChecksum: selectedArtifact?.sha256,
    approvedArtifactChecksum: approvedDecision ? selectedArtifact?.sha256 : undefined,
...
function buildCloseoutRequest(input: PropagationInput, artifact: ReviewArtifactEvidence): PublishingTitleCloseoutRequest {
  return {
    ...
    currentStageArtifactId: artifact.artifactId,
    approvalBoundArtifactId: artifact.artifactId,
    currentStageArtifactChecksum: artifact.sha256 || '',
    approvalBoundArtifactChecksum: artifact.sha256 || '',

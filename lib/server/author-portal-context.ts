  ...
  for (const opportunity of relatedOpportunities) {
    ...
    const summary = asset
      ? await dataverseFirst(config, 'jm1pub_editorialsummaries', {
          $select:
            'jm1pub_editorialsummaryid,jm1pub_summarybody,jm1pub_nextactionlabel,jm1pub_summarystatus,jm1pub_publishedtoworkspaceon,_jm1pub_sourceartifactid_value,createdon',
          $filter: `_jm1pub_publishingassetid_value eq ${dataverseLookupId(asset, 'jm1pub_publishingassetid')}`,
          $orderby: 'createdon desc',
        })
      : null
    ...
    const pendingGate = asset
      ? await dataverseFirst(config, 'jm1pub_editorialapprovalgates', {
        $select:
            'jm1pub_editorialapprovalgateid,jm1pub_gatecode,jm1pub_gatestatus,jm1pub_authordecision,jm1pub_authorresponsesummary,jm1pub_authordecisionon,jm1pub_nextstageauthorized,jm1pub_approvalstatusdetail,_jm1pub_deliverableartifactid_value,createdon',
          $filter: `_jm1pub_publishingassetid_value eq ${dataverseLookupId(asset, 'jm1pub_publishingassetid')}`,
          $orderby: 'createdon desc',
        })
      : null
...
async function getAuthorFacingEditorialArtifacts(
  ...
  return rows
    .filter((row) => stringValue(row.jm1pub_repositoryitemid) && stringValue(row.jm1pub_repositorydriveid))
    .map((row) => {
      const id = dataverseLookupId(row, 'jm1pub_editorialartifactid')
      const filename = stringValue(row.jm1pub_filename)
      const typeLabel = dataverseFormatted(row, 'jm1pub_artifacttype', 'Editorial artifact')
      const name = stringValue(row.jm1pub_editorialartifactname)

      return {
        id,
        filename,
        typeLabel,
        label: authorArtifactLabel({ name, filename, typeLabel }),
        href: `/api/author/artifacts/${id}/download`,
      }
    })
}

function authorArtifactLabel({
  name,
  filename,
  typeLabel,
}: {
  ...
  if (combined.includes('working manuscript')) return 'Editorial Working Manuscript'
  if (combined.includes('developmental review package') || combined.includes('author delivery pdf')) {
    return 'Developmental Review Summary'
  }
  if (combined.includes('revision blueprint')) return 'Revision Blueprint'
  return typeLabel || 'Editorial artifact'
}
...
function buildDevelopmentFallbackContext(session: AuthorPortalSession, overrides: ResolveOverrides): AuthorPortalContext {
  const projects = [
      ...
      summary: 'Developmental planning is being prepared for Volume I of the approved quarterly series.',
      nextActionLabel:

...
function determineWave3Rule(item, systemAttentionClass) {
  ...
  if (systemAttentionClass === 'MISSING_ARTIFACT') {
    return taskRule('Create structured operator task to bind current governed artifact/checksum', 'Artifact authority must be repaired before movement')
  }
  ...
  if (systemAttentionClass === 'MISSING_COMMERCIAL_STATE') {
    ...
    return taskRule('Create structured operator task to reconcile commercial state and payment/agreement gap', 'Commercial state gap needs deterministic binding')
  }

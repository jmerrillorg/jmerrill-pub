...
function namedTitleOverride(item, sourceIndex) {
  ...
  if (title.includes('indomitable')) {
    if (item.commercialLifecycleState?.includes('JOINED_THE_FAMILY') && item.titleLifecycleStage === 'EDITORIAL_PRODUCTION') {
      return null
    }
...
function inferRootCause(item, duplicateGroup) {
  if (item.systemAttention === 'MISSING_ARTIFACT_AUTHORITY') return 'MISSING_ARTIFACT_AUTHORITY'
  if (item.systemAttention === 'MISSING_CANONICAL_TITLE_LINK') return 'MISSING_CANONICAL_TITLE_LINK'
  if (item.systemAttention === 'MISSING_AUTHOR_DECISION') return 'MISSING_AUTHOR_DECISION'
  if (item.systemAttention === 'MISSING_COMMERCIAL_STATE') return 'MISSING_COMMERCIAL_STATE'
  if (item.systemAttention === 'RUNTIME_NOT_COMMISSIONED') return 'RUNTIME_NOT_COMMISSIONED'
  if (item.systemAttention === 'RUNTIME_FAILURE') return 'RUNTIME_FAILURE'
...
function refineNextAction(item, rootCause, automationClass) {
  ...
  if (rootCause === 'MISSING_ARTIFACT_AUTHORITY' && item.nextGovernedAction && item.nextGovernedAction !== 'Resolve lifecycle evidence gap') {
    return item.nextGovernedAction
  }
  const actions = {
